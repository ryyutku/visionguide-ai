# ultrasonic.py
#
# Backends (auto-selected):
#   "gpio"  — Raspberry Pi, RPi.GPIO
#   "stub"  — No hardware (laptop testing / simulation)
#
# Wiring (Pi):
#   HC-SR04 VCC  → Pi 5V   (pin 2)
#   HC-SR04 GND  → Pi GND  (pin 6)
#   HC-SR04 TRIG → GPIO 23 (BCM) — change via ULTRASONIC_TRIG env var
#   HC-SR04 ECHO → GPIO 24 (BCM) — change via ULTRASONIC_ECHO env var
#   !! Use a 1kΩ/2kΩ voltage divider on the ECHO line (Pi GPIO is 3.3V) !!
#
# Usage:
#   sensor = UltrasonicSensor()
#   cm     = sensor.read_distance_cm()   # float or None
#   band   = sensor.proximity_band(cm)   # "critical"|"close"|"medium"|"far"|"none"
#   sensor.close()

import os
import time
import threading
import logging

log = logging.getLogger("ultrasonic")

# Distance thresholds in cm — tune these for your environment
DIST_CRITICAL = 40    # stop immediately
DIST_CLOSE    = 80    # close, but not emergency
DIST_MEDIUM   = 150   # medium range
DIST_MAX      = 400   # ignore sensor noise beyond this


def _detect_backend() -> str:
    explicit = os.environ.get("ULTRASONIC_BACKEND", "").lower()
    if explicit in ("gpio", "stub"):
        return explicit
    try:
        with open("/proc/device-tree/model") as f:
            if "raspberry" in f.read().lower():
                return "gpio"
    except (FileNotFoundError, PermissionError):
        pass
    return "stub"


# ── Base ─────────────────────────────────────────────────────────────────────

class _SensorBase:
    def read_distance_cm(self) -> float | None:
        raise NotImplementedError

    def proximity_band(self, cm: float | None) -> str:
        if cm is None:
            return "none"
        if cm < DIST_CRITICAL:
            return "critical"
        if cm < DIST_CLOSE:
            return "close"
        if cm < DIST_MEDIUM:
            return "medium"
        return "far"

    def close(self):
        pass


# ── Raspberry Pi GPIO ─────────────────────────────────────────────────────────

class _GPIOSensor(_SensorBase):
    TRIG = int(os.environ.get("ULTRASONIC_TRIG", "23"))
    ECHO = int(os.environ.get("ULTRASONIC_ECHO", "24"))

    def __init__(self):
        import RPi.GPIO as GPIO
        self._gpio = GPIO
        GPIO.setmode(GPIO.BCM)
        GPIO.setwarnings(False)
        GPIO.setup(self.TRIG, GPIO.OUT)
        GPIO.setup(self.ECHO, GPIO.IN)
        GPIO.output(self.TRIG, False)
        time.sleep(0.05)

        # Background thread so readings never block the camera loop
        self._latest: float | None = None
        self._lock    = threading.Lock()
        self._running = True
        self._thread  = threading.Thread(target=self._loop, daemon=True)
        self._thread.start()
        log.info("GPIO ultrasonic ready  TRIG=%d  ECHO=%d", self.TRIG, self.ECHO)

    def _loop(self):
        while self._running:
            cm = self._read_once()
            with self._lock:
                self._latest = cm
            time.sleep(0.06)   # ~16 Hz

    def _read_once(self) -> float | None:
        GPIO = self._gpio
        GPIO.output(self.TRIG, True)
        time.sleep(0.00001)
        GPIO.output(self.TRIG, False)

        deadline = time.time() + 0.02
        while GPIO.input(self.ECHO) == 0:
            if time.time() > deadline:
                return None
        t0 = time.time()

        deadline = time.time() + 0.04
        while GPIO.input(self.ECHO) == 1:
            if time.time() > deadline:
                return None
        t1 = time.time()

        cm = (t1 - t0) * 17150
        return cm if cm < DIST_MAX else None

    def read_distance_cm(self) -> float | None:
        with self._lock:
            return self._latest

    def close(self):
        self._running = False
        self._thread.join(timeout=1)
        self._gpio.cleanup()


# ── Stub (laptop / no hardware) ───────────────────────────────────────────────

class _StubSensor(_SensorBase):
    """
    Oscillates between 30–200 cm so you can watch the guidance logic react.
    Pin it to a fixed value with:  ULTRASONIC_STUB_FIXED=120
    """
    def __init__(self):
        self._start = time.time()
        self._fixed = None
        v = os.environ.get("ULTRASONIC_STUB_FIXED")
        if v:
            try:
                self._fixed = float(v)
            except ValueError:
                pass
        mode = f"fixed {self._fixed} cm" if self._fixed else "oscillating 30–200 cm"
        log.info("Stub ultrasonic active (%s)", mode)

    def read_distance_cm(self) -> float | None:
        if self._fixed is not None:
            return self._fixed
        import math
        t  = time.time() - self._start
        return 115 + 85 * math.sin(t * 2 * math.pi / 20)


# ── Public factory ────────────────────────────────────────────────────────────

class UltrasonicSensor(_SensorBase):
    def __init__(self):
        backend = _detect_backend()
        log.info("Ultrasonic backend: %s", backend)
        self._impl = _GPIOSensor() if backend == "gpio" else _StubSensor()

    def read_distance_cm(self) -> float | None:
        return self._impl.read_distance_cm()

    def proximity_band(self, cm: float | None) -> str:
        return self._impl.proximity_band(cm)

    def close(self):
        self._impl.close()